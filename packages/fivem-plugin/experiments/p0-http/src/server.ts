/**
 * FiveAI P0 host-feasibility experiment: an MCP Streamable HTTP endpoint that
 ...
 * (§5, stage P0). P0 is a blocking technical probe, not a product slice: it
 * exists to answer five questions on a real FXServer, and it is deliberately
 * kept out of the delivery whitelist.
 *
 ...
 *
 * The RFC's proposed technical values (config v2 shape, §4.2 session
 * parameters) were frozen as this unit's baseline before it was written; they
 * live in `./frozen.ts`. Everything else here is probe scaffolding and is not
 * a published contract — the delivered resource replaces it with config v2,
 * the real tool catalog and the shared FIFO.
 */
...
const HTTP_PORT = resolveHttpPort(process.env["FIVEAI_P0_HTTP_PORT"]);
...
function report(tag: string, payload: unknown): void {
  console.log(`FIVEAI_P0 ${tag} ${JSON.stringify(payload)}`);
}
 ...
 * answer decides whether isolated compilation is possible or whether P0 has
...
/*
 * Tools (probe scaffolding). The delivered resource exposes the RFC §6.1
 * catalog with strict output schemas; P0 only needs enough surface to prove
 * the transport, the host-tick crossing and the compiler.
 */
...
const TOOLS = [
  {
    name: "p0_status",
    description:
      "P0 probe evidence: phase, listener address, session counts, boundary counters, runtime capabilities. Read-only and never queued.",
...
const DEFAULT_COMPILE_SOURCE = [
  ...
  "  const parts = [input.label, 'p0'].filter(Boolean);",
  "  return parts.join(':');",
  "}",
].join("\n");
...
function createMcpServer(): McpServer {
  const server = new McpServer(
    { name: "fiveai-p0-http", version: "0.0.1" },
    { capabilities: { tools: {} } },
  );
...
report("load", { resource: RESOURCE, resourceEpoch: RESOURCE_EPOCH, build: __P0_BUILD__ });
