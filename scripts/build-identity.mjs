 ...
 *   - scripts/*.mjs (the repository build scripts)
 *   - both packages' package.json
 *   - packages/mcp/tsconfig.json and packages/mcp/src/** (except src/generated/)
 *   - packages/fivem-plugin scripts/**, server/**, client/**, shared/**,
 *     and fxmanifest.lua
 *
 ...
 * `fiveai-mcp/<package version>/<sha256 hex>` where the version comes from
 * packages/mcp/package.json.
 *
 * writeBuildIdentity(root) embeds the identity into
 * packages/mcp/src/generated/build-identity.ts — a build artifact: git
...
const GENERATED_RELATIVE_PATH = "packages/mcp/src/generated/build-identity.ts";
...
const SINGLE_FILES = [
  ...
  "pnpm-lock.yaml",
  "pnpm-workspace.yaml",
  "packages/fivem-plugin/fxmanifest.lua",
  "packages/fivem-plugin/package.json",
  "packages/mcp/package.json",
...
const SOURCE_DIRECTORIES = [
  { directory: "scripts", recursive: false, extension: ".mjs", skip: [] },
  { directory: "packages/fivem-plugin/client", recursive: true, extension: null, skip: [] },
  { directory: "packages/fivem-plugin/scripts", recursive: true, extension: null, skip: [] },
  { directory: "packages/fivem-plugin/server", recursive: true, extension: null, skip: [] },
  { directory: "packages/fivem-plugin/shared", recursive: true, extension: null, skip: [] },
  { directory: "packages/mcp/src", recursive: true, extension: null, skip: ["generated"] },
];
...
export function getBuildIdentity(root) {
  ...
  let manifest;
  try {
    manifest = JSON.parse(readInput(root, "packages/mcp/package.json"));
  } catch {
    throw new Error("packages/mcp/package.json is not valid JSON");
  }
  if (manifest === null || typeof manifest !== "object" || typeof manifest.version !== "string" || manifest.version.length === 0) {
    throw new Error("packages/mcp/package.json has no usable version string");
  }
  return {
    packageVersion: manifest.version,
    buildId: `fiveai-mcp/${manifest.version}/${hash.digest("hex")}`,
  };
}
...
// Only a direct `node scripts/build-identity.mjs` invocation writes files;
// importing the module (including from contexts without a script argument,
// e.g. `node -e`) must never run the CLI or write anything.
