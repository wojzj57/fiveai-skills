 ...
 * build-relevant source tree — the root manifests, scripts/, and packages/
 * (never node_modules, dist, or artifact outputs) — plus the unified pack
 * test and this helper into a fresh mkdtemp directory, then prepares
 * isolated dependencies with `pnpm install --offline --frozen-lockfile`.
 * Build/pack scenarios run inside the fixture; the source repository
 * working tree is never written. Pass { install: false } for hash-only
 * fixtures (build-identity tests): the tree is copied but nothing is
 * installed, skipping the multi-minute offline install.
 *
 * The pack test and this helper are copied so a fixture can host a nested
 * `node --test tests/unified-pack.test.mjs` run, but the outer preservation
 * test is deliberately NOT copied — child workspaces cannot recurse into
 * further test runs. Only Node builtins are used; there are no new
 * dependencies.
 */
...
const SOURCE_DIRECTORIES = ["scripts", "packages"];
...
export function createUnifiedFixture(sourceRoot, { install = true } = {}) {
  ...
  const installDir = join(root, "dist", "fiveai-mcp");
  const zipPath = join(root, "dist", "fiveai-mcp.zip");
  ...
  try {
    for (const file of [...ROOT_FILES, ...TEST_FILES]) {
