 ...
 * packages/mcp/tests/process/broker-process.test.ts.
 */
...
const GENERATED_RELATIVE = "packages/mcp/src/generated/build-identity.ts";
...
test("the same source in different directories (no .git) yields one identity", () => {
  ...
  try {
    ...
    // Identity policy: fiveai-mcp/<package version>/<sha256 hex>.
    assert.match(identityA.buildId, new RegExp(`^fiveai-mcp/${identityA.packageVersion}/[0-9a-f]{64}$`));
  } finally {
    fixtureB.dispose();
    fixtureA.dispose();
  }
...
test("resource source edits change the build id; runtime files never do", () => {
  ...
  try {
    ...
    const serverPath = join(fixture.root, "packages/fivem-plugin/server/main.js");
    ...
    writeFileSync(
      join(fixture.root, "packages/fivem-plugin/mcp/credentials.json"),
      JSON.stringify({ entryToken: "runtime", bridgeToken: "runtime" }),
    );
    writeFileSync(
      join(fixture.root, "packages/fivem-plugin/mcp/config.json"),
...
test("writeBuildIdentity embeds exact literals, skips identical rewrites, and importing never writes", async () => {
  ...
  try {
    ...
    const generator = await import(pathToFileURL(join(fixture.root, "scripts", "build-identity.mjs")).href);
    ...
    const cli = spawnSync(process.execPath, [join(fixture.root, "scripts", "build-identity.mjs")], { encoding: "utf8" });
    assert.equal(cli.status, 0, `CLI stderr: ${cli.stderr}`);
    assert.match(cli.stdout, /^build-identity: fiveai-mcp\//);
    assert.equal(readFileSync(generatedPath, "utf8"), expected);

    rmSync(join(fixture.root, "pnpm-workspace.yaml"));
    const failed = spawnSync(process.execPath, [join(fixture.root, "scripts", "build-identity.mjs")], { encoding: "utf8" });
