...
function writeUnpackedConfig(unpackedRoot: string, port: number, serverLabel: string): void {
  writeFileSync(join(unpackedRoot, "fivem-mcp", "mcp", "config.json"), JSON.stringify({
    version: 1,
    broker: { host: "127.0.0.1", port },
    serverLabel,
    verifyEnabled: false,
...
function spawnUnpackedEntry(unpackedRoot: string): { child: ChildProcess; mcp: McpClient } {
  const child = spawn(process.execPath, [join(unpackedRoot, "fivem-mcp", "mcp", "entry.mjs")], {
    stdio: ["pipe", "pipe", "pipe"],
  });
  return { child, mcp: new McpClient(child) };
}
...
test("mixed A/B unified artifacts: the broker rejects the foreign build with BUILD_MISMATCH (F3)", async () => {
  ...
  const stateA = join(sideA, "fivem-mcp", "mcp", "state");
  const stateB = join(sideB, "fivem-mcp", "mcp", "state");
  ...
  try {
    ...
    assert.equal(existsSync(join(sideA, "fivem-mcp", "node_modules")), false, "side A is standalone");
    assert.equal(existsSync(join(sideB, "fivem-mcp", "node_modules")), false, "side B is standalone");
    ...
    harness = runServerBundle(join(sideA, "fivem-mcp", "dist", "server.js"), join(sideA, "fivem-mcp"));
    ...
    harness = runServerBundle(join(sideB, "fivem-mcp", "dist", "server.js"), join(sideB, "fivem-mcp"));
    ...
    writeFileSync(join(sideA, "fivem-mcp", "mcp", "credentials.json"), readFileSync(join(sideB, "fivem-mcp", "mcp", "credentials.json")));
