/**
 * Simulated FiveM server runtime for the HTTP MCP resource.
 *
 ...
 *   - `FIVEAI_MCP …` console lines are captured and parsed into evidence, so a
...
function parseEvidence(lines) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const match = /^FIVEAI_MCP (\S+) (\{.*\})$/.exec(lines[index]);
...
export function createFiveMHost({ bundlePath, resourceName = "fivem-mcp", resourcePath = null, env = {} } = {}) {
  ...
  const workDir = mkdtempSync(join(tmpdir(), "fiveai-mcp-host-"));
  ...
  async function waitForLine(tag, { from = 0, timeoutMs = 15_000 } = {}) {
    ...
    for (;;) {
      ...
      if (Date.now() >= deadline) {
        throw new Error(
          `timed out after ${timeoutMs}ms waiting for FIVEAI_MCP ${tag}\n` +
            `captured lines:\n${lines.join("\n")}`,
        );
      }
      await delay(20);
