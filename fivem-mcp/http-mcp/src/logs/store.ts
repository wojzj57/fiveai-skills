...
export class LogStore {
  available=false;
  serverHistoryReason:string|null=null;
  private lines:LogLine[]=[];
  private sizes:number[]=[];
  private bytes=0;
  private sequence=0n;
  private dropped=0;
  append(channel:string,raw:string,clientId?:number,messageText?:string):void {
    if(raw.includes('FIVEM_MCP_BIND:'))return;
    ...
    const message=Buffer.from(messageText??raw,'utf8').subarray(0,16384).toString('utf8').replace(/\x1b\[[0-9;]*m/g,'').replace(/\^[0-9]/g,'');
    const line:LogLine={sequence:String(++this.sequence),collectedAt:new Date().toISOString(),side:clientId===undefined?'server':'client',resource,channel:channel.slice(0,256),message,raw,forwarded:/^forwarded:/.test(channel),truncated,...(clientId===undefined?{}:{clientId})};
  ...
  coverage(side:'server'|'client'='server',clientId?:number,state?:string,reason:string|null=null) {
    ...
    if(side==='server'&&this.serverHistoryReason){state??=this.available?'partial':'unavailable';reason??=this.serverHistoryReason;}
