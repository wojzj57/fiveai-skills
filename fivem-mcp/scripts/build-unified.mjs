#!/usr/bin/env node
/**
 * Unified artifact orchestration (unified-artifact RFC §3, §7; HTTP RFC §12.2,
 * single-package design §5).
 *
 *   node fivem-mcp/scripts/build-unified.mjs publish   publish the staged
 *                                             delivery into dist/fivem-mcp/
 *                                             (no ZIP)
 *   node fivem-mcp/scripts/build-unified.mjs pack      stage fresh, publish the
 *                                             candidate directory and produce
 *                                             a validated dist/fivem-mcp.zip
 *
 * Only `pack` produces the candidate directory and the ZIP; `build` never
 * publishes. Both subcommands stage into dist/.staging-fivem-mcp/ from the
 * package outputs temporarily; staging is removed on both success and failure.
 * Staging uses the repository default config, after resolving and verifying
 * every staging/cleanup path inside the repository output root.
 *
 * A candidate output that already exists is reused only while it still looks
 * like a builder-owned output: any credentials file, a non-empty state
 * directory, a config that differs from the shipped default, or any file
 * outside the delivery whitelist makes the command refuse (non-zero) and ask
 * for a new candidate output path. The guard never clears the directory, so
 * the user's config, credentials, state, and other files stay byte-for-byte
 * intact — "preserve" is honoured by refusing, not by overwriting in place.
 *
 * Publishing replaces only whitelisted program files (same-directory temp +
 * rename). Packing zips the clean staging tree — never the candidate
 * directory — validates the ZIP entry set and bytes against the staging tree,
 * and only then replaces the final ZIP. Any failure exits non-zero without a
 * success message.
 */
...
const installDir = join(outputDir, "fivem-mcp");
const stagingDir = join(outputDir, ".staging-fivem-mcp");
const zipPath = join(outputDir, "fivem-mcp.zip");
const tempZipPath = join(outputDir, ".fivem-mcp.zip.tmp");
 ...
 */
const PROGRAM_FILES = [
  { src: "fivem-mcp/fxmanifest.lua", dest: "fxmanifest.lua" },
  { src: "fivem-mcp/README.md", dest: "README.md" },
  { src: "fivem-mcp/src/lua/executor.lua", dest: "shared/executor.lua" },
  { src: "fivem-mcp/dist/server.js", dest: "dist/server.js" },
  { src: "fivem-mcp/dist/client.js", dest: "dist/client.js" },
  { src: "fivem-mcp/dist/entry.mjs", dest: "mcp/entry.mjs" },
  { src: "fivem-mcp/dist/broker.mjs", dest: "mcp/broker.mjs" },
  { src: "fivem-mcp/dist/windows-files.ps1", dest: "mcp/windows-files.ps1" },
];
const DEFAULT_CONFIG = { src: "fivem-mcp/config/config.example.json", dest: "mcp/config.json" };
const DELIVERY_FILES = [...PROGRAM_FILES, DEFAULT_CONFIG];
const DELIVERY_DESTS = new Set(DELIVERY_FILES.map((file) => file.dest));
...
/**
 * Refuse to reuse a candidate output that shows any sign of being a used or
 * deployed install (HTTP RFC §12.2:250). The refusal keeps the user's config,
 * credentials, state and other files byte-for-byte intact and tells them to
 * choose a new candidate output path; the directory is never cleared. A
 * candidate output that does not exist yet is always publishable, so a first
 * install still works.
 */
function assertPublishableCandidate() {
  if (!existsSync(installDir)) return;
  // installDir lives inside outputDir, so both exist and can be resolved.
  assertWithinOutput(installDir, "install directory");
  const stat = lstatSync(installDir);
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    fail(`refusing to replace a candidate output that is not a real directory: ${installDir}`);
  }
  const configPath = join(installDir, DEFAULT_CONFIG.dest);
  if (existsSync(configPath)) {
    assertRealFile(configPath, "the existing candidate config");
    if (Buffer.compare(readFileSync(configPath), readFileSync(join(repoRoot, DEFAULT_CONFIG.src))) !== 0) {
      fail(
        `the candidate output holds a locally modified ${DEFAULT_CONFIG.dest}; ` +
          `choose a new candidate output path instead of replacing it: ${installDir}`,
      );
    }
  }
  for (const name of listFilesRecursive(installDir)) {
    if (!DELIVERY_DESTS.has(name)) {
      fail(
        `the candidate output holds a file outside the delivery whitelist (${name}); ` +
          `choose a new candidate output path instead of replacing it: ${installDir}`,
      );
    }
  }
...
/**
 * Publish the staged program files into dist/fivem-mcp/ (RFC §7): the
 * candidate output is first confirmed to still be a builder-owned, unused
 * output, then program files are replaced one by one. Cross-file replacement
 * is not transactional — a mid-way failure leaves earlier files updated and
...
function publishStaged() {
  ...
  if (existsSync(configDest)) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink()) {
      fail(`the candidate mcp/config.json is not a regular file: ${configDest}`);
    }
    // The guard already proved it matches the shipped default; leave it as is.
  } else {
...
/**
 * Publish the candidate directory and build the ZIP from the clean staging
 * tree (never from the candidate directory), validate it in a sibling temp
 * file, and only then replace the final ZIP. On failure the temp file is
 * removed and the previous ZIP (if any) stays — explicitly not this run's
 * product.
 */
function packZip() {
  publishStaged();
  const entries = {};
  for (const file of DELIVERY_FILES) {
    entries[`fivem-mcp/${file.dest}`] = new Uint8Array(readFileSync(join(stagingDir, file.dest)));
  }
  ...
  try {
    ...
    for (const [name, bytes] of Object.entries(reread)) {
      if (!name.startsWith("fivem-mcp/")) {
        fail(`ZIP validation failed: entry outside the fivem-mcp root: ${name}`);
      }
      const staged = new Uint8Array(readFileSync(join(stagingDir, name.slice("fivem-mcp/".length))));
...
function main() {
  ...
  console.error("usage: node fivem-mcp/scripts/build-unified.mjs publish|pack");
  process.exitCode = 2;
}
