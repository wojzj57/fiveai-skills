...
if(process.argv.includes('--help')){
 console.log('node scripts/benchmark-http-mcp.mjs [http://127.0.0.1:30130/mcp]\nRuns 20 samples per class. Measures end-to-end native JS tool and concurrent status latency.');
 process.exit(0);
}
...
const client=new Client({name:'fiveai-js-benchmark',version:'1.0.0'});
const transport=new StreamableHTTPClientTransport(url);
const summaries=[];
try{
 await client.connect(transport);
 const before=await client.callTool({name:'status',arguments:{}});
 if(before.structuredContent?.data?.javascript?.mode!=='native')throw new Error('Native JavaScript execution is required');
 ...
 for(const sample of cases){
  ...
  for(let index=0;index<20;index++){
   ...
   const execution=client.callTool({name:'execute_js',arguments:{side:'server',code:sample.code}});
   ...
   if(sample.name==='deep-syntax-error'?value?.error?.code!=='JAVASCRIPT_INVALID':!value?.ok||task?.state!=='succeeded')throw new Error('Unexpected task result: '+JSON.stringify(value));
   await delay(100);
  }
 ...
 console.log(JSON.stringify({kind:'complete',buildId:before.structuredContent.data.buildId,summaries,note:'End-to-end tool latency, including queue and execution; not isolated parsing cost.'}));
