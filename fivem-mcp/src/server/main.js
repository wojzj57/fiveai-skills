...
import { McpConfigSchema, resolveConfigPaths, validateCredentialFile } from "../shared/config.ts";
import { BUILD_ID } from "../generated/build-identity.ts";
...
function connect() {
  ...
  ws.on("close", code => {
    ...
    if (code === BUILD_MISMATCH_CLOSE_CODE) {
      ...
      phase = "incompatible";
      console.log("[fivem-mcp] MCP bridge version mismatch; upgrade or reinstall both the desktop entry and the FiveM resource, then restart the resource");
      return;
    }
...
setTick(() => {
  ...
  for (let n = 0; n < 32 && incoming.length; n++) {
    ...
    if (item.kind === "identity") {
      ...
      else console.log("[fivem-mcp] OS process identity unavailable; continuing bridge startup without verified process restart detection");
      identityReady = true;
      continue;
    }
    ...
    if (item.kind === "config" || item.kind === "config-error") {
      ...
      if (item.kind === "config-error") {
        phase = "invalid";
        console.log(`[fivem-mcp] mcp config unavailable (${item.code}); the installation looks incomplete - fix it and restart the resource`);
        continue;
      }
      try {
        ...
        phase = "invalid";
        console.log("[fivem-mcp] mcp config invalid (not valid JSON or failed schema validation); fix mcp/config.json and restart the resource");
      }
      continue;
    }
      ...
      if (item.kind === "credentials-error") {
        if (item.code === "ENOENT") {
          ...
          if (now - lastCredentialNoticeAt >= 30000) {
            ...
            console.log("[fivem-mcp] mcp credentials not present yet; waiting for first-run initialization by the desktop entry");
          }
          scheduleCredentialRead([1000, 2000, 4000, 8000, 10000][credentialAttempts++] ?? 10000);
        } else {
          phase = "invalid";
          console.log(`[fivem-mcp] mcp credentials unreadable (${item.code}); fix the file access and restart the resource`);
        }
        continue;
      }
      try {
        ...
        phase = "invalid";
        console.log("[fivem-mcp] mcp credentials invalid (corrupt or wrong form); fix or remove the file and restart the resource");
      }
      continue;
    }
...
RegisterCommand("fiveai_mcp_verify", (sender, args) => {
  ...
  if (verificationBusy) { console.log("[fivem-mcp] verification still running or unresolved"); return; }
  const target = args[0] ?? "server";
  const client = target === "server" ? null : clients.get(Number(target));
  if (target !== "server" && !client?.ready) { console.log("[fivem-mcp] client is not bound"); return; }
  ...
  })().catch(() => { console.log("[fivem-mcp] verification unresolved; inspect host logs"); });
}, true);
